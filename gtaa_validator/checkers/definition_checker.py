...
class DefinitionChecker(BaseChecker):
    ...
    def add_violation(
        ...
        """
        Añadir una violación a la lista (legacy).
...
# Legacy visitor for backwards compatibility with Python AST
    ...
    def visit_Call(self, node: ast.Call):
        if self.inside_test_function:
            if isinstance(node.func, ast.Attribute):
                ...
                is_selenium_call = method_name in DefinitionChecker.BROWSER_METHODS_PYTHON
                is_browser_object = object_name in DefinitionChecker.BROWSER_OBJECTS_PYTHON

                if is_selenium_call and is_browser_object:
